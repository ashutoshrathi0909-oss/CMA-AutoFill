...
def find_precedents(firm_id: str, source_term: str, entity_type: str) -> List[PrecedentMatch]:
    ...
    # We query all precedents for this firm + all global precedents
    # In a larger app, we'd filter via RPC or limit. For MVP, fetch all relevant.
    res = db.table("classification_precedents").select("*").or_(f"firm_id.eq.{firm_id},scope.eq.global").execute()
...
def create_precedent(firm_id: str, source_term: str, target_row: int, target_sheet: str, entity_type: str, scope: str, project_id: str, user_id: str):
    db = get_supabase()
    # Unique constraint handler (firm_id, source_term, entity_type)
    # Check if exists
    existing = db.table("classification_precedents").select("id").eq("firm_id", firm_id).eq("source_term", source_term).eq("entity_type", entity_type).execute()
    ...
    if existing.data:
        res = db.table("classification_precedents").update(payload).eq("id", existing.data[0]["id"]).execute()
        return res.data[0]
    else:
