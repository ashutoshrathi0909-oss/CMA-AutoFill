...
def classify_project(project_id: str, firm_id: str, entity_type: str) -> ClassificationResult:
    ...
    for doc in ["profit_and_loss", "balance_sheet", "trial_balance"]:
        if data.get(doc) and data[doc].get("line_items"):
            for itm in data[doc]["line_items"]:
                # skip pure totals and empty
                if not itm.get("is_total") and itm.get("name"):
                    all_raw_items.append({
                        "item_name": itm["name"],
                        "item_amount": itm["amount"],
                        "document_type": doc
                    })
    ...
    for item in all_raw_items:
        ...
        if prec and prec.confidence >= 0.80:
            results.append(ClassifiedItem(
                ...
                needs_review=(prec.confidence < 0.70)
            ))
            continue
    ...
    if to_ai:
        client = GeminiClient()
        model_name = os.getenv("LLM_CLASSIFICATION_MODEL", "gemini-3-flash")
        ...
        precedents_str = "[]" # we'd feed recent global firm precs here but skipping to save tokens if we verified Tier1 locally 
    ...
    stats = ClassificationResult(
        ...
        classified_by_ai=sum(1 for r in results if r.source == "ai" or r.source == "ai_uncertain"),
        unclassified=sum(1 for r in results if r.confidence < 0.30),
